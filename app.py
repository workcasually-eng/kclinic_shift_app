...
def staff_screen():
    ...
    # ----------------------------------------------------------------
        ...
        if not df_log.empty and user_name in df_log.columns:
            ...
            # 処理対象年(target_y)のログが存在するか確認し、なければ現在の暦年にフォールバック
            if df_log[df_log['dt_obj'].dt.year == target_y].empty:
                current_cal_year = datetime.date.today().year
                if not df_log[df_log['dt_obj'].dt.year == current_cal_year].empty:
                    eval_year = current_cal_year
            
            # 選択された年のデータのみ抽出
            current_year_logs = df_log[df_log['dt_obj'].dt.year == eval_year]
        ...
        # 3. メトリクス表示
        st.markdown(f"**📊 {eval_year}年度 休日状況**")
